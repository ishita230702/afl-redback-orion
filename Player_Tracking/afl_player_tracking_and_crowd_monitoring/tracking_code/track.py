...
class Tracking:
    ...
    def __call__(self, video_path: str, output_video: str = None, save_csv: bool = True) -> Dict:
        return self.process_video(video_path, output_video, save_csv)
    ...
    def save_tracking_csv(self, tracking_data: dict, video_path: str, out_dir: str = "outputs") -> str:
        """Save tracking results to CSV for heatmap pipeline."""
        base_name = os.path.splitext(os.path.basename(video_path))[0]
        csv_output_path = os.path.join(out_dir, f"{base_name}_tracking.csv")
        ...
        with open(csv_output_path, "w", newline="") as csvfile:
            ...
            for frame in tracking_data["tracking_results"]:
                for player in frame["players"]:
                    writer.writerow([
                        frame["frame_number"], player["player_id"], frame["timestamp"],
    ...
    def process_video(self, video_path: str, output_path: str = None, save_csv: bool = True) -> Dict:
        ...
        if save_csv:
            csv_path = self.save_tracking_csv(tracking_data, video_path, out_dir="outputs")
            print(f"[INFO] Tracking results saved to CSV: {csv_path}")
...
def main():
    ...
    # Process video with output video and CSV saving
