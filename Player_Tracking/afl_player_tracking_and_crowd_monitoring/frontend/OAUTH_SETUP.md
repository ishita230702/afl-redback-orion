...
## Prerequisites
- Active accounts with Google Cloud Console and Apple Developer Program
...
2. Choose "External" user type (for non-Google Workspace orgs)
...
Set these environment variables in your application (local `.env` and your deployment environment):
```bash
...
- Invalid `redirect_uri`: Ensure the redirect URI matches exactly
- Access blocked: Add your domain to authorized domains in OAuth consent screen
- API not enabled: Enable the OAuth2 API in Google Cloud Console
